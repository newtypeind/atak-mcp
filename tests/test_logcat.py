...
from atak_mcp.bridge import device
...
def test_grep_searches_full_buffer(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(device, "adb", _fake_adb(buf, calls))

    out = device.logcat(lines=200, grep="MARKER")
...
def test_lines_caps_after_filtering(monkeypatch):
    ...
    monkeypatch.setattr(device, "adb", _fake_adb(buf, []))

    out = device.logcat(lines=3, grep="MARKER")
...
def test_no_filter_uses_fast_tail_path(monkeypatch):
    calls = []
    monkeypatch.setattr(device, "adb", _fake_adb("a\nb\nc\nd\ne", calls))

    out = device.logcat(lines=2)
...
def test_since_count_maps_to_dash_t(monkeypatch):
    calls = []
    monkeypatch.setattr(device, "adb", _fake_adb("x\ny\nz", calls))

    device.logcat(since="500")
...
def test_since_timestamp_is_a_single_arg(monkeypatch):
    calls = []
    monkeypatch.setattr(device, "adb", _fake_adb("hit MARKER", calls))

    device.logcat(since="06-05 09:00:00.000", grep="MARKER")
...
def test_lines_zero_means_no_cap(monkeypatch):
    ...
    out = device.logcat(lines=0, grep="MARKER")

    assert len(out.splitlines()) == 50


def test_backward_compatible_grep_and_lines(monkeypatch):
    monkeypatch.setattr(device, "adb", _fake_adb("foo\nMARKER hit\nbar", []))
    assert device.logcat(lines=200, grep="MARKER") == "MARKER hit"
